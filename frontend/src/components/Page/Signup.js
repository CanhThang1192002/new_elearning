...
const Signup = () => {
    ...
    try {
      while (page < totalPages) {
        const res = await fetch(
          `http://localhost:8081/v1/api/user?page=${page}&size=10`,
          {
            method: "GET",
          }
        );
      ...
      console.log(
        "Tất cả userCode:",
        allUsers.map((user) => user.userCode).filter(Boolean)
      );
      console.log(
        "Tất cả email:",
      ...
      console.log(
        "Tất cả phone:",
        allUsers.map((user) => user.phone).filter(Boolean)
      );
  ...
  useEffect(() => {
    if (userCode != null) {
      ...
      if (
        trimmedUserCode.length < 3 ||
        trimmedUserCode.length > 20 ||
        !/^[a-zA-Z0-9]+$/.test(trimmedUserCode)
      ) {
        setUserCodeError(
          "Tên đăng nhập phải từ 3 đến 20 ký tự, chỉ bao gồm chữ cái và số."
        );
        setIsCheckingUserCode(false);
        return;
      }
      ...
      const timeout = setTimeout(async () => {
        try {
          ...
          const userExists = allUsers.some((user) => {
            const userCodeValue = user.userCode
              ? user.userCode.toLowerCase().trim()
              : "";
  ...
  const handleSignup = async () => {
    ...
    const payload = {
      ...
      statusCode: "ACTIVE",
      experience:
        role === "Giảng viên" && experience ? parseInt(experience) : null,
      certification:
        role === "Giảng viên" && selectedCertificates.length > 0
    ...
    <div style={styles.signupBgMain}>
      ...
      <header style={styles.signupHeader}>
        ...
        <div style={styles.signupNavContainer}>
          <nav style={styles.signupNavLinks}>
            <a href="#" style={styles.signupNavLink}>
              Về chúng tôi
            </a>
            <a href="#" style={styles.signupNavLink}>
              Khóa học
      ...
      <main style={styles.signupFormContainer}>
        ...
        <div style={styles.signupFormFields}>
          ...
          {isCheckingUserCode && (
            <div style={styles.errorText}>Đang kiểm tra...</div>
          )}
          {userCodeError && !isCheckingUserCode && (
            <div style={styles.errorText}>{userCodeError}</div>
          )}
          ...
          {phoneError && !isCheckingPhone && (
            <div style={styles.errorText}>{phoneError}</div>
          )}

          <div style={styles.signupFormGroup}>
            <label htmlFor="address" style={styles.signupFormLabel}>
              Địa chỉ:
            </label>
            <input
              type="text"
          ...
          <div style={styles.signupFormGroup}>
            <label htmlFor="dob" style={styles.signupFormLabel}>
              Ngày sinh:
            </label>
            <input
              type="date"
          ...
          {experienceError && (
            <div style={styles.errorText}>{experienceError}</div>
          )}
