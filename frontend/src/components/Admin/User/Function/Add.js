...
api.interceptors.response.use(
  ...
  (error) => {
    if (
      ...
      error.response?.data?.errorStatus === 907
    ) {
...
const AddUserModal = ({
  ...
  const validateForm = () => {
    ...
    if (!formData.phoneCode)
      newErrors.phoneCode = "Mã vùng điện thoại là bắt buộc";
  ...
  const validateResponseData = (responseData, sentData) => {
    ...
    if (responseData.data?.userCode !== sentData.userCode) {
      errors.push(
        `Mã định danh trả về (${responseData.data?.userCode}) không khớp với mã gửi (${sentData.userCode}).`
      );
    }
    if (responseData.data?.email !== sentData.email) {
      errors.push(
        `Email trả về (${responseData.data?.email}) không khớp với email gửi (${sentData.email}).`
      );
    }
    if (responseData.data?.roleId !== sentData.roleId) {
      errors.push(
        `Vai trò trả về (${responseData.data?.roleId}) không khớp với vai trò gửi (${sentData.roleId}).`
      );
    }
    if (responseData.data?.statusCode !== sentData.statusCode) {
      errors.push(
        `Trạng thái trả về (${responseData.data?.statusCode}) không khớp với trạng thái gửi (${sentData.statusCode}).`
      );
    }
    return errors;
  };
      ...
      try {
        ...
        if (data.errorStatus === 900) {
          ...
          if (validationErrors.length > 0) {
            toast.warn(
              `Thêm người dùng thành công nhưng có lỗi dữ liệu: ${validationErrors.join(
                " "
              )}`
            );
          } else {
          ...
          <div className="modal-grid">
            <div className="modal-grid-item">
              <label
                ...
                title="Mã định danh duy nhất cho người dùng"
              >
                Mã định danh:
              </label>
              <input
            ...
            <div className="modal-grid-item">
              <label
                className="field-label"
                title="Xác nhận mật khẩu phải khớp với mật khẩu"
              >
                Xác nhận mật khẩu:
              </label>
              <input
            ...
            <div className="modal-grid-item">
              <label
                className="field-label"
                title="Số điện thoại từ 9-15 chữ số"
              >
                Số điện thoại:
              </label>
            ...
            <div className="modal-grid-item horizontal-field">
              <label
                className="field-label"
                title="Chọn giới tính của người dùng"
              >
                Giới tính:
              </label>
            ...
            <div className="modal-grid-item horizontal-field">
              <label
                className="field-label"
                title="Trạng thái hoạt động của người dùng"
              >
                Trạng thái:
              </label>
              <div className="inline-options">
            ...
            <div className="modal-grid-item">
              <label
                className="field-label"
                title="Vai trò của người dùng trong hệ thống"
              >
                Vai trò:
              </label>
              <select
            ...
            <div className="modal-grid-item">
              <label
                className="field-label"
                title="Địa chỉ hiện tại của người dùng"
              >
                Địa chỉ:
              </label>
              <input
            ...
            {formData.role === "Giảng viên" && (
              <>
                <div className="modal-grid-item">
                  <label
                    className="field-label"
                    title="Chọn các chứng chỉ của giảng viên"
                  >
                    Chứng chỉ:
                  </label>
                ...
                <div className="modal-grid-item">
                  <label
                    className="field-label"
                    title="Số năm kinh nghiệm giảng dạy"
                  >
                    Năm kinh nghiệm:
                  </label>
                  <select
